import type {ChannelsChannel, ChannelsController, ChannelStatus} from '@repo/channels'
import {getQueryCacheKey, type PresentationAPI} from '@repo/visual-editing-helpers'
...
export interface LoaderQueriesProps {
  ...
  controller: ChannelsController<PresentationAPI> | undefined
  perspective: ClientPerspective
  documentsOnPage: {_id: string; _type: string}[]
  onLoadersConnection: Dispatch<SetStateAction<ChannelStatus>>
  onDocumentsOnPage: (
    key: string,
...
export default function LoaderQueries(props: LoaderQueriesProps): JSX.Element {
  ...
  const [channel, setChannel] = useState<ChannelsChannel<PresentationAPI, 'loaders'>>()
  ...
  useEffect(() => {
    if (controller) {
      ...
      channel.onStatus(onLoadersConnection)

      channel.on('documents', (data) => {
  ...
  useEffect(() => {
    if (channel) {
      ...
      channel.post('loader/perspective', {
        projectId: projectId!,
        dataset: dataset!,
        perspective: activePerspective,
      })
...
interface QuerySubscriptionProps
  ...
  query: string
  params: QueryParams
  channel: ChannelsChannel<PresentationAPI, 'loaders'>
}
function QuerySubscription(props: QuerySubscriptionProps) {
  ...
  useEffect(() => {
    if (resultSourceMap) {
      channel.post('loader/query-change', {
        projectId,
        dataset,
        perspective,
        query,
